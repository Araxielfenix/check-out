...
function numberValidation(event) {
	...
	if (event.value.length > 0) {
		if (
			isNaN(
				event.value.substring(event.value.length - 1, event.value.length)
			) &&
			event.value.substring(event.value.length - 1, event.value.length) != " "
		) {
	...
	if (event.value.length >= 4) {
		document.getElementById("card-number").textContent =
			event.value.substring(0, 4) +
			" " +
			event.value.substring(4, 8) +
			" " +
			event.value.substring(8, 12) +
			" " +
			event.value.substring(12, 16);
	}
...
function search4Logo(brand) {
	...
	fetch(
		"https://duckduckgo-image-search.p.rapidapi.com/search/image?q=Logo%20" +
			brand,
		options
	)
		.then((response) => response.json())
...
function cardStyle(brand, type, scheme, issuer) {
	...
	if (brand == "VISA") {
		...
		document.getElementById("card-front").style.background =
			"rgb(26 26 108 / 58%)";
		...
		document.getElementById("card-back").style.background =
			"rgb(26 26 108 / 58%)";
...
function updateCardCVV(event) {
	...
	if (event.value.length > 3) {
		document.getElementById("card-cvv").textContent = event.value.substring(
			0,
			3
		);
...
function flipCard(isFlipped) {
	if (!isFlipped) {
		document.getElementById("inner").classList.remove("flipped"); // Asegurarse de que la clase "flipped" esté removida
	} else {
		document.getElementById("card").classList.remove("flipped"); // Asegurarse de que la clase "flipped" esté removida
		document.getElementById("cvv-div").classList.remove("flipped"); // Asegurarse de que la clase "flipped" esté removida
		document.getElementById("inner").classList.add("flipped"); // Agregar la clase "flipped" para mostrar la parte delantera
	}
}
